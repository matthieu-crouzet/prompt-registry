...
describe('InstallationManager.pruneEmptyDirs Test Suite', () => {
    ...
    describe('pruneEmptyDirs - Directory Structure Cleanup', () => {
        it('should remove empty directories after file removal', async () => {
            ...
            expect(fs.existsSync(nestedDir), 'Nested directory should be removed').toBe(false);
            expect(fs.existsSync(path.join(extractionPath, 'prompt-registry-core', 'deep')), 'Deep directory should be removed').toBe(false);
            expect(fs.existsSync(path.join(extractionPath, 'prompt-registry-core')), 'prompt-registry-core directory should be removed').toBe(false);
            expect(fs.existsSync(extractionPath), 'Extraction path should be removed if empty').toBe(false);
        });
        ...
        it('should not remove directories that contain other files', async () => {
            ...
            expect(fs.existsSync(nestedDir), 'Empty nested directory should be removed').toBe(false);
            expect(fs.existsSync(path.join(extractionPath, 'prompt-registry-core')), 'Directory with remaining file should remain').toBe(true);
            expect(fs.existsSync(extractionPath), 'Extraction path should remain').toBe(true);
            expect(fs.existsSync(remainingFile), 'Remaining file should not be touched').toBe(true);
        });
        ...
        it('should handle metadata directory removal when separate from extraction', async () => {
            ...
            expect(fs.existsSync(extractionPath), 'Extraction path should be removed').toBe(false);
            expect(fs.existsSync(installPath), 'Installation/metadata path should be removed').toBe(false);
        });
        ...
        it('should gracefully handle non-existent installation', async () => {
            ...
            await expect(installationManager.pruneEmptyDirs(InstallationScope.USER)).resolves.not.toThrow();
        });
